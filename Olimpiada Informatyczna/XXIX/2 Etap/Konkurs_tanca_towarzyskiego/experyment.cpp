//I was experimenting on this code to debug wzo.cpp
#include <bits/stdc++.h>
using namespace std;
...
struct T2{
    ...
    void add(int a, int b, int val, int v=1, int l=1, int r=M)
    {
        if(r < a || b < l) return;
        if(a <= l && r <= b) 
        {
        ...
        int mid = (l + r) / 2;
        add(a, b, val, v*2, l, mid);
        add(a, b, val, v*2+1, mid+1, r);
    }
    int get_tree(int pos)
    {
        pos = pos + M - 1;
        int res = 0;
        while(pos)
...
int32_t main() 
{
    ...
    // // cout<<"-tin "<<tin[1]<<" -tout "<<tout[1]<<endl;
    // // cout<<"-tin "<<tin[2]<<" -tout "<<tout[2]<<endl;
    // // cout<<"-tin "<<tin[4]<<" -tout "<<tout[4]<<endl;
    // // cout<<"-tin "<<tin[5]<<" -tout "<<tout[5]<<endl;

    // for(int i=1;i<=it[0];++i)
    //     cout<<i<<"state["<<state[0][i]<<"]: "<<range[0][i].f<<" "<<range[0][i].s<<endl;
    ...
    for(int i = 1; i <= q; ++i)
    {
        ...
        if(c[i] == '?')
        {
            if(state[0][x[i]] == 0)
            {
                // cout<<"graf.get_tree("<<tin[0][range[0][x[i]].f]+1<<", "<<tin[0][range[0][x[i]].s]<<")"<<'\n';
